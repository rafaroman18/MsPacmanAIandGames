package com.github.rafaroman18.game;

import com.github.rafaroman18.game.controllers.ghosts.GhostsActions;
import com.github.rafaroman18.game.controllers.ghosts.IGhostsController;
import com.github.rafaroman18.game.controllers.ghosts.game.GameGhosts;
import com.github.rafaroman18.game.controllers.pacman.HumanPacMan;
...
import com.github.rafaroman18.game.core.GameView;
import com.github.rafaroman18.game.core.Replay;
import com.github.rafaroman18.game.core._G_;
...
public class PacManSimulator {
	...
	public static class GameConfig {
		...
		/**
		 * Total percentage of PILLS present within the level. If < 1, some (random)
		 * pills will be taken away.
		 */
	...
	public synchronized Game run(final SimulatorConfig config) {
		...
		if (config.game.seed <= 0) {
			...
			while (config.game.seed < 0)
				config.game.seed += Integer.MAX_VALUE;
		}
		...
		ThinkingThread pacManThread = new ThinkingThread("PAC-MAN", new IThinkingMethod() {
			@Override
			public void think() {
				PacManSimulator.this.config.pacManController.tick(game.copy(), due);
			}
		});
		ThinkingThread ghostsThread = new ThinkingThread("GHOSTS", new IThinkingMethod() {
			@Override
			public void think() {
				if (PacManSimulator.this.config.ghostsController != null)
					PacManSimulator.this.config.ghostsController.tick(game, due);
			}
		});
		...
		try {
			while (!game.gameOver()) {
				...
				GhostsActions ghostsActions = (config.ghostsController == null ? null
