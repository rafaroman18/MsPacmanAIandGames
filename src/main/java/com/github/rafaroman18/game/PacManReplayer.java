...
import com.github.rafaroman18.game.core.Replay.ReplayGhosts;
import com.github.rafaroman18.game.core.Replay.ReplayMsPacman;
import com.github.rafaroman18.game.core._RG_;
 ...
 * 
 * TODO: for some strange reasons, this is not working correctly ... problems
 * usually starts when PacMan eats power-pill... It seems that replays are not
 * correctly replayed ...
 * 
 * @author Jimmy
 */
	...
	public synchronized void run(final ReplayerConfig config) {
		...
		try {
			...
			while (!game.gameOver()) {
				...
				if (!paused || nextFrame) {
					nextFrame = false;
					game.advanceGameReplay(pacManAction, ghostsActions,
							((ReplayMsPacman) pacManController).getLocation(game),
							((ReplayGhosts) ghostController).getLocations(game));
				}
