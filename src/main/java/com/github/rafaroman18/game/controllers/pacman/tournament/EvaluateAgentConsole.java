...
public class EvaluateAgentConsole {
	...
	private static void initJSAP() throws JSAPException {
		...
		FlaggedOption opt1 = new FlaggedOption(ARG_AGENT_FQCN_LONG).setStringParser(JSAP.STRING_PARSER)
				.setRequired(true).setShortFlag(ARG_AGENT_FQCN_SHORT).setLongFlag(ARG_AGENT_FQCN_LONG);
		opt1.setHelp("Agent fully-qualified class name to evaluate. Must be present on classpath.");

		jsap.registerParameter(opt1);

		FlaggedOption opt2 = new FlaggedOption(ARG_AGNET_ID_LONG).setStringParser(JSAP.STRING_PARSER).setRequired(true)
				.setShortFlag(ARG_AGENT_ID_SHORT).setLongFlag(ARG_AGNET_ID_LONG);
		opt2.setHelp("Id of the agent that will serve as identificator within results file.");

		jsap.registerParameter(opt2);

		FlaggedOption opt3 = new FlaggedOption(ARG_ONE_RUN_REPETITIONS_LONG).setStringParser(JSAP.INTEGER_PARSER)
				.setRequired(false).setDefault("10").setShortFlag(ARG_ONE_RUN_REPETITIONS_SHORT)
				.setLongFlag(ARG_ONE_RUN_REPETITIONS_LONG);
		opt3.setHelp(
				"How many times should be one PacMan game configuration be repeated (in order to gain statistically sound data).");

		jsap.registerParameter(opt3);

		FlaggedOption opt31 = new FlaggedOption(ARG_SIMULATOR_OPTIONS_LONG).setStringParser(JSAP.STRING_PARSER)
				.setRequired(true).setShortFlag(ARG_SIMULATOR_OPTIONS_SHORT).setLongFlag(ARG_SIMULATOR_OPTIONS_LONG);
		opt31.setHelp("List of simulator options.");

		jsap.registerParameter(opt31);

		FlaggedOption opt32 = new FlaggedOption(ARG_RESULT_DIR_LONG).setStringParser(JSAP.STRING_PARSER)
				.setRequired(false).setDefault("./results").setShortFlag(ARG_RESULT_DIR_SHORT)
		...
		FlaggedOption opt33 = new FlaggedOption(ARG_RUNS_COUNT_LONG).setStringParser(JSAP.INTEGER_PARSER)
				.setRequired(false).setDefault("100").setShortFlag(ARG_RUNS_COUNT_SHORT)
		...
		FlaggedOption opt6 = new FlaggedOption(ARG_SEED_LONG).setStringParser(JSAP.INTEGER_PARSER).setRequired(false)
				.setDefault("0").setShortFlag(ARG_SEED_SHORT).setLongFlag(ARG_SEED_LONG);
	...
	public static String[] getTestArgs() {
		return new String[] { "-s", "20" // "seed"
				, "-o", "-pp false -tp 0.5 -gc 0 -lc 1 -v false -2x false -p false -tt 40 -r true" // prototype-options";
				, "-c", "5" // run-count
				, "-r", "5" // one-run-repetitions
				, "-p", "game.controllers.pacman.examples.NearestPillPacMan" // agent-fqcn ... requires MarioAI4J-Agents
																				// on classpath!
