...
import com.github.rafaroman18.game.controllers.pacman.tournament.run.PacManResults;
import com.github.rafaroman18.game.controllers.pacman.tournament.run.PacManRun;
import com.github.rafaroman18.game.controllers.pacman.tournament.run.PacManRunResult;
import com.github.rafaroman18.game.controllers.pacman.tournament.run.PacManRunsGenerator;
import com.github.rafaroman18.game.controllers.pacman.tournament.utils.Sanitize;
...
public class EvaluateAgent {
	...
	public EvaluateAgent(int seed, SimulatorConfig prototypeConfig, int runCount, int oneRunRepetitions,
			File resultDirFile) {
		this.seed = seed;
	...
	public void evaluateAgent(String agentId, String agentFQCN) {
		...
		log(agentId, "EVALUATING AGENT IN " + runCount + " RUNS with " + oneRunRepetitions + " repetition, TOTAL "
				+ (runCount * oneRunRepetitions) + " SIMULATIONS!");
		...
		for (int i = 0; i < runs.length; ++i) {
			...
			if (runs[i].getConfig().config.replay) {
				if (runs[i].getConfig().config.replayFile == null) {
					...
					runs[i].getConfig().config.replayFile = new File(
							runs[i].getConfig().config.replayFile.getParentFile(), newFile);
				}
			}
	...
	// System.out.println("[" + agentId + "] Outputing runs into: " +
	...
	//
	// writer.println("agentId;configNumber;simulationNumber;" +
	// results.getRunResults().get(0).getCSVHeader());
	...
	//
	// } catch (Exception e) {
	// throw new RuntimeException("Failed to write results into: " +
	// file.getAbsolutePath());
	// } finally {
