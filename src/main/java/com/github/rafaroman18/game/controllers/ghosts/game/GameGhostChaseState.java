...
public class GameGhostChaseState implements GameGhostAI {
	...
	public int[] execute(int ghostType, Game game, long timeDue) {
		...
		if (ghostType == CLYDE) {
			double distanceFromPac = GameGhosts.getEuclideanDistance(game.getX(game.getCurGhostLoc(ghostType)),
					game.getY(game.getCurGhostLoc(ghostType)), game.getX(game.getCurPacManLoc()),
