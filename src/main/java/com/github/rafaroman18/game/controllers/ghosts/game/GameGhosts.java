	...
	public void tick(Game game, long timeDue) {
		...
		for (int i = 0; i < Game.NUM_GHOSTS; i++) {
			if (game.getEdibleTime(i) > 0) {
				...
				// }
				GhostState[i] = FRIGHTENED;
				// System.out.println("In fright for " + i + "at edible time" +
				// game.getEdibleTime(i));
			...
			if (GhostState[i] != FRIGHTENED) {
				...
				if (Debugging) {
					GameView.addLines(game, color, game.getX(game.getCurGhostLoc(i)), game.getY(game.getCurGhostLoc(i)),
			...
			if (game.ghostRequiresAction(i) && GhostState[i] != FRIGHTENED) {
				...
				for (int j = 0; j < possibleDirections.length; j++) {
					...
					double distanceBetweenNeighbor = GameGhosts.getEuclideanDistance(game.getX(directionNodeNum),
							game.getY(directionNodeNum), ghostTarget[X], ghostTarget[Y]);
