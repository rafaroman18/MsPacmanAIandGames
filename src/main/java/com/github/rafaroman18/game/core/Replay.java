...
public class Replay {
    ...
    public static void saveActions(GameConfig gameConfig, int ghostCount, String actions, File replayFile,
            boolean firstWrite) {
        try {
    ...
    public class ReplayGhosts implements IGhostsController {
        ...
        public void tick(Game game, long timeDue) {
            ...
            for (int ghost = 0; ghost < actions.length; ++ghost) {
                ...
                Direction locationDir = Direction.NONE;
                for (int j = 0; j < 4; ++j) {
                    if (locationIndex == game.getNeighbour(game.getCurGhostLoc(ghost), j))
