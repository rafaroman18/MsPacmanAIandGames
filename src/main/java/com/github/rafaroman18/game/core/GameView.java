...
import com.github.rafaroman18.tools.Scale2x;
...
public final class GameView extends JComponent {
    ...
    public void paintComponent(Graphics g) {
        ...
        drawMaze();
        drawDebugInfo(); // this will be used during testing only and will be disabled in the competition
                         // itself
        drawPills();
        drawPowerPills();
        drawPacMan();
    ...
    private void drawPowerPills() {
        ...
        for (int i = 0; i < powerPillIndices.length; i++)
            if (game.checkPowerPill(i))
                bufferGraphics.fillOval(game.getX(powerPillIndices[i]) * MAG + 1,
                        game.getY(powerPillIndices[i]) * MAG + 5, 8, 8);
    }
    ...
    private void drawPacMan() {
        ...
        bufferGraphics.drawImage(pacmanImgs[pacManDir][(game.getTotalTime() % 6) / 2], game.getX(pacLoc) * MAG - 1,
                game.getY(pacLoc) * MAG + 3, null);
    }
        ...
        for (int index = 0; index < G.NUM_GHOSTS; index++) {
            ...
            if (game.getEdibleTime(index) > 0) {
                if (game.getEdibleTime(index) < _G_.EDIBLE_ALERT && ((game.getTotalTime() % 6) / 3) == 0)
                    bufferGraphics.drawImage(ghostsImgs[5][0][(game.getTotalTime() % 6) / 3], x * MAG - 1, y * MAG + 3,
                            null);
                else
                    bufferGraphics.drawImage(ghostsImgs[4][0][(game.getTotalTime() % 6) / 3], x * MAG - 1, y * MAG + 3,
                            null);
            } else {
                if (game.getLairTime(index) > 0)
                    bufferGraphics.drawImage(ghostsImgs[index][G.UP][(game.getTotalTime() % 6) / 3],
                            x * MAG - 1 + (index * 5), y * MAG + 3, null);
                else
                    bufferGraphics.drawImage(
                            ghostsImgs[index][game.getCurGhostDir(index)][(game.getTotalTime() % 6) / 3], x * MAG - 1,
                            y * MAG + 3, null);
            }
        }
    }
    ...
    private BufferedImage getImage(String fileName) {
        ...
        try {
            ...
            // File(pathImages+System.getProperty("file.separator")+fileName));
    ...
    public void setScale2x(boolean state) {
        ...
        if (state) {
            ...
            if (frame != null) {
                ...
                // frame.setPreferredSize(new Dimension(frame.getWidth() * 2, frame.getHeight()
                // * 2));
                frame.repaint();
            }
        } else {
    ...
    public GameView showGame() {
        ...
        if (scale2x != null) {
            ...
            // this.frame.setPreferredSize(new Dimension(frame.getWidth() * 2,
            // frame.getHeight() * 2));
            this.frame.center();
    ...
    // for debugging/illustration purposes only: draw colors in the maze to check
    // whether controller is working
    ...
    /**
     * Add points of certain colors to node indices (highlights certain nodes) <br/>
     * <br/>
     * Must be called every frame, i.e., points are deleted after they are drawn.
     * <br/>
     * <br/>
     * NOTE: This won't do anything in the competition but your code will still
     * work.
     * 
     * @param game
     * @param color
     ...
     * (fromNnodeIndices.length must be equals toNodeIndices.length) <br/>
     * <br/>
    ...
    public synchronized static void addLines(Game game, Color color, int fromNnodeIndex, int toNodeIndex) {
        if (isVisible)
            debugLines.add(new DebugLine(game.getX(fromNnodeIndex), game.getY(fromNnodeIndex), game.getX(toNodeIndex),
                    game.getY(toNodeIndex), color));
    }

    /**
     * Adds a line to be drawn between specified nodes for the purpose of PATH
     * visualization, i.e., taking care of TORUS on X-AXIS. <br/>
     * <br/>
     * Must be called every frame, i.e., lines are deleted after they are drawn.
     * <br/>
     * <br/>
    ...
    /**
     * Adds a line leading between [fromX;fromY] -> [toX;toY] in pixels. <br/>
     * <br/>
     * Must be called every frame, i.e., lines are deleted after they are drawn.
     * <br/>
     * <br/>
