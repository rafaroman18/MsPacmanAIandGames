...
public interface Game {
	...
	public static final float EDIBLE_TIME_REDUCTION = 0.9f; // reduction factor by which edible time decreases as level
															// number increases
	public static final int[] LAIR_TIMES = { 40, 60, 80, 100 }; // time spend in the lair by each ghost at the start of
																// a level
	...
	public static final int[] INITIAL_GHOST_DIRS = { 3, 3, 3, 3 }; // initial directions for the ghosts (after leaving
																	// the lair)
	...
	public int[] advanceGame(PacManAction pacMan, GhostsActions ghosts); // advances the game using the actions
																			// (directions) supplied; returns all
																			// directions played [PacMan, Ghost1,
																			// Ghost2, Ghost3, Ghost4]

	public int getReverse(int direction); // returns the reverse of the direction supplied

	public boolean gameOver(); // returns true is Ms Pac-Man has lost all her lives or if MAX_LEVELS has been
								// reached
	...
	public int[] getPacManNeighbours(); // returns an array of size 4, indicating neighbouring nodes for the current
										// position of Ms Pac-Man. E.g., [-1,12,-1,44] for neighbours 12 and 44 in
										// direction RIGHT and LEFT

	public int[] getGhostNeighbours(int whichGhost); // returns an array of size 4, indicating neighbouring nodes for
														// the current position of the ghost specified. Replaces the
														// direction corresponding to the opposite previous direction
														// with -1
	...
	public int getEdibleTime(int whichGhost); // returns the edible time (time left in which the ghost can be eaten) for
												// the ghost specified
	...
	public int getNumberPills(); // returns the total number of pills in this maze (at the beginning of the
									// level)

	public int getNumberPowerPills(); // returns the total number of power pills in this maze (at the beginning of the
										// level)

	public int getLairTime(int whichGhost); // returns the time remaining the ghost specified spends in the lair

	public boolean ghostRequiresAction(int whichGhost); // returns true of ghost is at a junction and a direction is
														// needed
	...
	public int getInitialGhostsPosition(); // returns the position where the ghosts starts at the beginning of the
											// level, AFTER leaving the lair
	...
	public int getPillIndex(int nodeIndex); // returns the pill index of the node specified (can be used with the bitset
											// for the pills)

	public int getPowerPillIndex(int nodeIndex); // returns the power pill index of the node specified (can be used with
													// the bitset for the power pills)

	public int getNeighbour(int nodeIndex, int direction); // returns the neighbour of the node specified for the
															// direction supplied
	...
	public enum DM {
		PATH, EUCLID, MANHATTEN
	}; // simple enumeration for use with the direction methods (below)

	public int getNextPacManDir(int to, boolean closer, DM measure); // returns the direction Ms Pac-Man should take to
																		// approach/retreat from the node specified,
																		// using the distance measure specified

	public int getNextGhostDir(int whichGhost, int to, boolean closer, DM measure); // returns the direction the ghost
																					// specified should take to
																					// approach/retreat from the node
																					// specified, using the distance
																					// measure specified

	public int getPathDistance(int from, int to); // returns the shortest path distance (Dijkstra) from one node to
													// another
	...
	public int[] getPossiblePacManDirs(boolean includeReverse); // returns the set of possible directions for Ms
																// Pac-Man, with or without the direction opposite to
																// the last direction taken

	public int[] getPossibleGhostDirs(int whichGhost); // returns the set of possible directions for the ghost specified
	...
	public int[] getGhostPath(int whichGhost, int to); // returns the path from one node to another, taking into account
														// that reversals are not possible

	public int getTarget(int from, int[] targets, boolean nearest, DM measure); // selects a target from 'targets' given
																				// current position ('from'), a distance
																				// measure and whether it should be the
																				// point closest or farthest

	public int getGhostTarget(int from, int[] targets, boolean nearest); // selects a target for a ghost (accounts for
																			// the fact that ghosts may not reverse)

	public int getGhostPathDistance(int whichGhost, int to); // returns the distance of a path for the ghost specified
																// (accounts for the fact that ghosts may not reverse)
}