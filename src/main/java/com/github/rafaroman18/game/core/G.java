 ...
 */
package com.github.rafaroman18.game.core;

import com.github.rafaroman18.game.PacManSimulator.GameConfig;
import com.github.rafaroman18.game.controllers.Direction;
import com.github.rafaroman18.game.controllers.ghosts.GhostsActions;
import com.github.rafaroman18.game.controllers.pacman.PacManAction;
...
public class G implements Game {
	...
	public int[] advanceGame(PacManAction pacMan, GhostsActions ghosts) {
		...
		// This is primarily done for the replays as reset (as possibly called by
		// feast()) sets the
		// last directions to the initial ones, not the ones taken
		int[] replayStep = { lastPacManDir, lastGhostDirs[0], lastGhostDirs[1], lastGhostDirs[2], lastGhostDirs[3],
	...
	// Central method that advances the game state
	public int[] advanceGameReplay(PacManAction pacMan, GhostsActions ghosts, int pacManLocation,
			int[] ghostsLocations) {
		// updatePacMan(pacMan); //move pac-man
	...
	// Checks the direction supplied by the controller and substitutes for a legal
	// one if necessary
	...
	// Checks the directions supplied by the controller and substitutes for a legal
	// ones if necessary
	...
	protected boolean eatPowerPill() {
		...
		if (powerPillIndex >= 0 && powerPills.get(powerPillIndex)) {
			...
			// This ensures that only ghosts outside the lair (i.e., inside the maze) turn
			// edible
			...
			// This turns all ghosts edible, independent on whether they are in the lair or
			// not
	...
	// Checks the state of the level/game and advances to the next level or
	// terminates the game
	...
	// Returns the neighbours of the node at which the specified ghost currently
	// resides. NOTE: since ghosts are not allowed to reverse, that
	// neighbour is filtered out. Alternatively use: getNeighbour(), given
	// curGhostLoc[-] for all directions
	public int[] getGhostNeighbours(int whichGhost) {
		int[] neighbours = Arrays.copyOf(mazes[curMaze].graph[curGhostLocs[whichGhost]].neighbours,
	...
	public boolean ghostRequiresAction(int whichGhost) {
		return (isJunction(curGhostLocs[whichGhost])
				&& (edibleTimes[whichGhost] == 0 || edibleTimes[whichGhost] % GHOST_SPEED_REDUCTION != 0));
	}
	...
	// Returns the starting position of the ghosts (i.e., first node AFTER leaving
	// the lair)
	public int getInitialGhostsPosition() {
		return mazes[curMaze].initialGhostsPosition;
	}

	// Total number of nodes in the graph (i.e., those with pills, power pills and
	...
	// Returns the power pill index of the node. If it is -1, the node has no pill.
	// Otherwise one
	...
	// Returns the neighbour of node index that corresponds to direction. In the
	// case of neutral, the
	...
	// Returns the number of neighbours of a node: 2, 3 or 4. Exception: lair, which
	// has no neighbours
	...
	// Returns the direction Pac-Man should take to approach/retreat a target (to)
	// given some distance
	// measure
	...
	// Returns the direction the ghost should take to approach/retreat a target (to)
	// given some distance
	...
	// This method returns the direction to take given some options (usually
	// corresponding to the
	// neighbours of the node in question), moving either towards or away (closer in
	// {true, false})
	...
	// Returns the path of adjacent nodes from one node to another, including these
	// nodes
	...
	// Similar to getPath(-) but takes into consideration the fact that ghosts may
	// not reverse. Hence the path to be taken
	...
	// Returns the node from 'targets' that is closest/farthest from the node 'from'
	// given the distance measure specified
	...
	// Returns the target closes from the position of the ghost, considering that
	...
	// Returns the path distance for a particular ghost: takes into account the fact
	// that ghosts may not reverse
	public int getGhostPathDistance(int whichGhost, int to) {
		return getGhostPath(whichGhost, to).length;
	}

	// Returns the neighbours of a node with the one correspodining to the reverse
	// of direction being deleted (i.e., =-1)
	...
	/*
	 * Stores the actual mazes, each of which is simply a connected graph. The
	 * differences between the mazes are the connectivity and the x,y coordinates
	 * (used for drawing or to compute the Euclidean distance. There are 3 built-in
	 * distance functions in total: Euclidean, Manhatten and Dijkstra's shortest
	 * path distance. The latter is pre-computed and loaded, the others are computed
	 * on the fly whenever getNextDir(-) is called.
	 */
	protected final class Maze {
		...
		protected int initialPacPosition, lairPosition, initialGhostsPosition, width, height; // Maze-specific
																								// information
		protected String name; // Name of the Maze

		/*
		 * Each maze is stored as a (connected) graph: all nodes have neighbours, stored
		 * in an array of length 4. The index of the array associates the direction the
		 * neighbour is located at: '[up,right,down,left]'. For instance, if node '9'
		 * has neighbours '[-1,12,-1,6]', you can reach node '12' by going right, and
		 * node 6 by going left. The directions returned by the controllers should thus
		 * be in {0,1,2,3} and can be used directly to determine the next node to go to.
		 */
		...
		private void loadNodes(String fileName) {
			try {
				...
				// InputStreamReader(this.getClass().getResourceAsStream("/data/"+fileName)));
				// APPLICATION

				// ORIG:
				// BufferedReader br=new BufferedReader(new InputStreamReader(new
				// FileInputStream(pathMazes+System.getProperty("file.separator")+fileName)));
				BufferedReader br = new BufferedReader(
						new InputStreamReader(this.getClass().getResourceAsStream("resources/data/" + fileName)));
				...
				while (input != null) {
					...
					Node node = new Node(nd[0], nd[1], nd[2], nd[7], nd[8],
							new String[] { nd[3], nd[4], nd[5], nd[6] });
		 ...
		 * contains the shortest distance from any node in the maze to any other node.
		 * Since the graph is symmetric, the symmetries have been removed to preserve
		 * memory and all distances are stored in a 1D array; they are looked-up using
		 * getDistance(-).
		 */
	...
	/*
	 * Stores all information relating to a node in the graph, including all the
	 * indices required to check and update the current state of the game.
	 */
	protected final class Node {
		protected int x, y, nodeIndex, pillIndex, powerPillIndex, numNeighbours;
		protected int[] neighbours;

		protected Node(String nodeIndex, String x, String y, String pillIndex, String powerPillIndex,
				String[] neighbours) {
